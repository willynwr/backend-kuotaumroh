<div :style="editFreelanceModalOpen ? 'display: flex' : 'display: none'" class="fixed inset-0 z-50 items-center justify-center">
  <div class="absolute inset-0 bg-black/40" @click="closeEditFreelanceModal()"></div>
  <div class="relative z-10 w-full max-w-4xl rounded-lg bg-white shadow-lg max-h-[90vh] overflow-y-auto">
    ...
    <div class="sticky top-0 z-10 bg-white border-b px-6 py-4">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-muted-foreground">Update data freelance</p>
        </div>
        <button @click="closeEditFreelanceModal()" class="text-muted-foreground hover:text-foreground transition-colors">
    ...
    <form method="POST" :action="`/admin/freelance/${editingFreelance.id}`" enctype="multipart/form-data" class="p-6 space-y-6">
      @csrf
      @method('PUT')
      <input type="hidden" name="redirect_to" value="/admin/users?tab=freelance">
      <input type="hidden" name="provinsi" x-bind:value="editingFreelance.province">
      <input type="hidden" name="kab_kota" x-bind:value="editingFreelance.city">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="nama" x-model="editingFreelance.name" required
          ...
          <div>
            ...
            <input type="email" name="email" x-model="editingFreelance.email" required
              class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
          </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="tel" name="no_wa" x-model="editingFreelance.phoneClean" @input="editingFreelance.phoneClean = editingFreelance.phoneClean.replace(/[^0-9]/g, '')" required placeholder="81xxx"
          ...
          <div>
            ...
            <input type="text" name="link_referral" x-model="editingFreelance.referral_code" required
              class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
          </div>
        </div>
      </div>
      ...
      <div class="space-y-4">
          ...
          <div x-data="{ provinceDropdownOpen: false, provinceSearch: '' }">
            ...
            <div class="relative" @click.away="provinceDropdownOpen = false">
              <button type="button" @click="provinceDropdownOpen = !provinceDropdownOpen"
                ...
                <span :class="!editingFreelance.province && 'text-muted-foreground'" x-text="editingFreelance.province || 'Pilih provinsi'"></span>
              ...
              <div x-show="provinceDropdownOpen" x-transition class="absolute z-50 mt-1 w-full rounded-md border border-input bg-white shadow-lg max-h-60 overflow-y-auto">
                <template x-for="province in provinces.filter(p => p.toLowerCase().includes(provinceSearch.toLowerCase()))" :key="province">
                  <button type="button" @click="editingFreelance.province = province; loadCitiesForEdit(province); provinceDropdownOpen = false"
          ...
          <div x-data="{ cityDropdownOpen: false }">
            ...
            <div class="relative" @click.away="cityDropdownOpen = false">
              <button type="button" @click="cityDropdownOpen = !cityDropdownOpen"
                class="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
                <span :class="!editingFreelance.city && 'text-muted-foreground'" x-text="editingFreelance.city || 'Pilih kota/kabupaten'"></span>
              ...
              <div x-show="cityDropdownOpen" x-transition class="absolute z-50 mt-1 w-full rounded-md border border-input bg-white shadow-lg max-h-60 overflow-y-auto">
                <template x-for="city in cities" :key="city">
                  <button type="button" @click="editingFreelance.city = city; cityDropdownOpen = false"
        ...
        <div>
          ...
          <textarea name="alamat_lengkap" x-model="editingFreelance.address" required rows="3"
        ...
        <!-- KTP Upload Section -->
        <div class="space-y-2" x-data="{ ktpPreview: editingFreelance.ktp_url ? '/storage/' + editingFreelance.ktp_url : null, ktpType: editingFreelance.ktp_url ? (editingFreelance.ktp_url.endsWith('.pdf') ? 'application/pdf' : 'image') : null, isNewFile: false }">
          ...
          <div x-show="ktpPreview && isNewFile" class="mt-3">
            ...
            <div x-show="ktpType && ktpType.startsWith('image/')" class="relative inline-block">
              ...
              <button type="button" @click="ktpPreview = editingFreelance.ktp_url ? '/storage/' + editingFreelance.ktp_url : null; ktpType = editingFreelance.ktp_url ? (editingFreelance.ktp_url.endsWith('.pdf') ? 'application/pdf' : 'image') : null; isNewFile = false; $el.closest('.space-y-2').querySelector('input[type=file]').value = ''"
