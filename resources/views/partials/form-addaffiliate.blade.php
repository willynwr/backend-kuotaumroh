<div x-show="addAffiliateModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center"
  ...
  <div class="relative z-10 w-full max-w-2xl rounded-lg bg-white shadow-lg p-6 max-h-[90vh] overflow-y-auto"
    ...
    <form x-ref="addAffiliateForm" method="POST" action="{{ route('admin.affiliates.store') }}" @submit.prevent="openConfirmAddModal()" class="mt-6 space-y-4">
      @csrf
            ...
            <div x-show="cityDropdownOpen" x-transition class="absolute z-50 mt-1 w-full rounded-md border border-input bg-white shadow-lg" style="display: none;">
              ...
              <div class="max-h-60 overflow-y-auto">
                <template x-for="city in cities.filter(c => c.toLowerCase().includes(citySearch.toLowerCase()))" :key="city">
                  <button type="button" @click="newAffiliate.kab_kota = city; cityDropdownOpen = false; citySearch = ''"
      ...
      <div class="flex items-center justify-end gap-3 pt-4">
        <button type="button" @click="closeAddAffiliateModal()" class="h-9 px-4 rounded-md border text-sm font-medium text-slate-700 hover:bg-muted transition-colors">Batal</button>
