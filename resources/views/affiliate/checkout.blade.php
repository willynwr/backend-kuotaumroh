        ...
        <div x-show="paymentStatus === 'expired'" x-cloak class="flex items-center justify-center min-h-[60vh]">
            <div class="max-w-md w-full rounded-lg border bg-white shadow-sm">
                <div class="p-6 text-center space-y-4">
                    ...
                    <div class="pt-4">
                        <button @click="window.location.href = '{{ route('affiliate.order') }}'"
        ...
        <div x-show="['pending', 'verifying', 'activated'].includes(paymentStatus)" x-cloak class="overflow-hidden">
            ...
            <div class="mb-6">
                <div class="flex items-start gap-4">
                    <a href="#" @click.prevent="handleUiBack('{{ route('affiliate.order') }}')" class="inline-flex h-10 w-10 items-center justify-center rounded-md border bg-white hover:bg-muted transition-colors">
                    ...
                    <div class="rounded-lg border bg-white shadow-sm">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                ...
                                <p class="text-sm text-yellow-700">Harap membayar sesuai nominal (termasuk kode unik).</p>
                            </div>
                            ...
                            <button @click="handleCheckPayment()" class="w-full inline-flex items-center justify-center gap-2 rounded-md bg-primary text-primary-foreground h-11 px-4 py-2 hover:bg-primary/90 font-medium mt-4">
                                ...
                                <span x-text="paymentStatus === 'activated' ? 'Terkonfirmasi' : 'Cek Status Pembayaran'"></span>
                            </button>
                    ...
                    <div class="rounded-lg border bg-white shadow-sm">
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="mt-6 pt-6 border-t space-y-3">
                                ...
                                <button @click="window.location.href = '/dash/{{ $linkReferral ?? '' }}'" class="w-full inline-flex items-center justify-center gap-2 rounded-md border bg-background h-10 px-4 py-2 hover:bg-muted">
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showSuccessModal"
                ...
                <div class="bg-white px-6 py-5 space-y-3">
                    ...
                    <button @click="redirectToDashboard()" type="button" 
    ...
    return {
        ...
        async init() {
            this.$watch('useStaticQris', async (value) => {
                if (value && !this.qrisStaticDynamicString) await this.fetchStaticDynamicQris();
                this.generateQRCode();
            });
            ...
            if (data.paymentStatus && ['verifying', 'activated'].includes(data.paymentStatus)) {
        ...
        confirmExit() {
            ...
            window.location.href = '{{ route("affiliate.order") }}';
        },
        ...
        async autoVerifyPayment() {
            ...
            try {
                ...
                if (v.success && ['berhasil','success','sukses'].includes(v.status?.toLowerCase())) { console.log('✅ Verify found payment successful!'); this.setPaymentActivated(); return; }
                ...
                if (d?.id) {
                    ...
                    else if (['pending','unpaid'].includes(st) || st.includes('menunggu')) { console.log('⚠️ API returned pending, keeping:', this.paymentStatus); }
        ...
        qrCodeInstance: null,
        generateQRCode() {
            const qris = this.useStaticQris ? this.qrisStaticString : this.qrisString;
            if (!qris) return;
        ...
        startPaymentPolling() {
            ...
            this.paymentCheckInterval = setInterval(async () => {
                try {
                    // Step 1: Trigger verifyPayment untuk cek mutasi QRIS
                    const v = await verifyPayment(this.paymentId);
                    console.log('🔄 Polling verify response:', v);
                    if (v.success && ['berhasil','success','sukses'].includes(v.status?.toLowerCase())) { console.log('✅ Polling: Payment successful!'); this.setPaymentActivated(); return; }
                    // Step 2: Get payment data dari database
                    const r = await getPaymentStatus(this.paymentId);
                    const d = Array.isArray(r) ? r[0] : (r.data || r);
                    const st = (d?.status || d?.payment_status || '').toLowerCase();
                    ...
                    if (d?.id) {
                        ...
                        else if (['pending','unpaid'].includes(st) || st.includes('menunggu')) { console.log('⚠️ Polling: keeping verifying'); }
        ...
        async handleCheckPayment() {
            ...
            this.showToast('Mengecek', 'Sedang mengecek...');
            try {
                const v = await verifyPayment(this.paymentId);
        ...
        redirectToDashboard() {
            this.isForceExit = true;
            window.location.href = '/dash/{{ $linkReferral ?? "" }}';
        },
