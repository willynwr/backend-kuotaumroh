...
<style>
  .sticky-column { position: sticky; left: 0; z-index: 10; background: white; }
  .sticky-column::after { content: ''; position: absolute; top: 0; right: 0; bottom: 0; width: 1px; background: #e5e7eb; }
  thead th.sticky-column { z-index: 20; }
  tbody tr:hover .sticky-column { background: #f9fafb; }
  .table-content { font-size: 16px; }
  .table-content td, .table-content th { font-size: 16px; }
  .sticky-pagination { background: white; border-top: 1px solid #e5e7eb; }
  ...
  .badge { display: inline-flex; align-items: center; padding: 4px 12px; border-radius: 9999px; font-size: 13px; font-weight: 500; border: 1px solid transparent; }
  ...
  .package-status { text-align: left; font-size: 14px; line-height: 1.6; }
    ...
    <div class="rounded-lg border bg-white shadow-sm">
      <div class="p-6">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          ...
          <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            ...
            <div class="relative">
              ...
              <input type="text" :placeholder="viewMode === 'batch' ? 'Cari batch/nama travel/agen/affiliate/tanggal/nominal' : 'Cari nomor/provider/paket/harga/travel/agen/affiliate'" x-model="search" class="flex h-10 rounded-md border border-input bg-background pl-9 pr-3 py-2 text-sm w-full sm:w-[300px]">
            </div>
            <!-- Status Filter -->
        ...
        <div x-show="viewMode === 'batch'">
              ...
              <thead>
                <tr class="border-b">
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[180px]">
                    <button @click="handleSort('travelName')" class="inline-flex items-center gap-2">
                      Nama Travel
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleSort('territory')" class="inline-flex items-center gap-2">
                      Teritori
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[180px]">
                    <button @click="handleSort('agentName')" class="inline-flex items-center gap-2">
                      Nomor Agen
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[180px]">
                    <button @click="handleSort('affiliateName')" class="inline-flex items-center gap-2">
                      Affiliate
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleSort('marginTotal')" class="inline-flex items-center gap-2">
                      Margin
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
              ...
              <tbody>
                <template x-for="tx in paginatedTransactions" :key="tx.id">
                  <tr class="border-b transition-colors hover:bg-muted/50">
                    ...
                    <td class="p-4 align-middle whitespace-nowrap min-w-[180px]">
                      <div class="flex flex-col">
                        <span class="font-medium" x-text="tx.affiliateName || '-'"></span>
                        <span class="text-muted-foreground" x-text="tx.affiliatePhone || ''"></span>
                      </div>
                    </td>
                ...
                <template x-if="sortedTransactions.length === 0">
                  <tr>
                    <td colspan="13" class="p-8 text-center text-muted-foreground">Tidak ada transaksi yang ditemukan</td>
                  </tr>
                </template>
              </tbody>
            </table>
          ...
          <!-- Pagination Controls for Batch View -->
          <div class="sticky-pagination flex flex-col lg:flex-row items-center justify-between gap-4 p-4" x-show="viewMode === 'batch'">
            ...
            <div class="flex items-center gap-2">
              <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1" :class="currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-muted'" class="inline-flex items-center justify-center rounded-md border bg-transparent h-8 px-3 transition-colors">
                Previous
              </button>
              ...
              <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages" :class="currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-muted'" class="inline-flex items-center justify-center rounded-md border bg-transparent h-8 px-3 transition-colors">
                Next
              </button>
            </div>
        ...
        <div x-show="viewMode === 'number'" x-cloak>
            ...
            <table class="w-full table-content">
              <thead>
                <tr class="border-b">
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap sticky-column min-w-[180px]">
                    <button @click="handleItemSort('msisdn')" class="inline-flex items-center gap-2">
                      Nomor HP
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[150px]">
                    <button @click="handleItemSort('provider')" class="inline-flex items-center gap-2">
                      Provider
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleItemSort('packageName')" class="inline-flex items-center gap-2">
                      Paket
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-center align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[120px]">
                    <button @click="handleItemSort('status')" class="inline-flex items-center gap-2">
                      Status
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleItemSort('price')" class="inline-flex items-center gap-2">
                      Harga
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleItemSort('margin')" class="inline-flex items-center gap-2">
                      Margin
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[200px]">
                    <button @click="handleItemSort('travelName')" class="inline-flex items-center gap-2">
                      Nama Travel
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleItemSort('territory')" class="inline-flex items-center gap-2">
                      Teritori
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap min-w-[180px]">
                    <button @click="handleItemSort('agentName')" class="inline-flex items-center gap-2">
                      Nomor Agen
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleItemSort('affiliateName')" class="inline-flex items-center gap-2">
                      Nama Affiliate
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
                  ...
                  <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground whitespace-nowrap">
                    <button @click="handleItemSort('createdAt')" class="inline-flex items-center gap-2">
                      Tanggal & Waktu
                      <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                      </svg>
              ...
              <tbody>
                <template x-for="(item, idx) in paginatedItems" :key="idx">
                  <tr class="border-b transition-colors hover:bg-muted/50">
                    ...
                    <td class="p-4 align-middle whitespace-nowrap">
                      <div class="flex flex-col">
                        <span class="font-medium" x-text="item.affiliateName || '-'"></span>
                        <span class="text-muted-foreground" x-text="item.affiliatePhone || ''"></span>
                      </div>
                    </td>
                ...
                <template x-if="sortedItems.length === 0">
                  <tr>
                    <td colspan="12" class="p-8 text-center text-muted-foreground">Tidak ada data yang ditemukan</td>
                  </tr>
                </template>
              </tbody>
            </table>
          ...
          <!-- Pagination Controls for Number View -->
...
function transactionsPage() {
  return {
    ...
    get filteredTransactions() {
      return this.transactions.filter(tx => {
        ...
        const searchable = [
          ...
          String(tx.items?.length || 0),
          this.formatRupiah(tx.totalAmount),
    ...
    get sortedTransactions() {
      return [...this.filteredTransactions].sort((a, b) => {
        ...
        switch (this.sortKey) {
          ...
          case 'affiliateName': comparison = (a.affiliateName || '').localeCompare(b.affiliateName || ''); break;
    ...
    get filteredItems() {
      const allItems = this.transactions.flatMap(tx =>
        (tx.items || []).map(item => ({
          ...item,
          batchId: tx.batchId,
          batchName: tx.batchName,
          createdAt: tx.createdAt,
          travelName: tx.travelName,
          territory: tx.territory || '',
          agentName: tx.agentName || '',
          agentPhone: tx.agentPhone || '',
          affiliateName: tx.affiliateName || '',
          affiliatePhone: tx.affiliatePhone || '',
        }))
      );
    ...
    handleItemSort(key) {
      if (this.itemSortKey === key) {
        ...
        this.itemSortDirection = key === 'createdAt' ? 'desc' : 'asc';
      }
    },
    ...
    handleDetailSort(key) {
      if (this.detailSortKey === key) {
        ...
        this.detailSortDirection = key === 'msisdn' ? 'asc' : 'desc';
      }
    },
    ...
    getPackageStatusSummary(transaction) {
      ...
      const counts = { berhasil: 0, proses: 0, gagal: 0 };
      transaction.items.forEach(item => {
        const status = (item.status || '').toLowerCase();
        if (status === 'berhasil' || status === 'selesai' || status === 'completed' || status === 'success') {
          counts.berhasil++;
        } else if (status === 'proses' || status === 'processing') {
          counts.proses++;
        } else if (status === 'gagal' || status === 'failed' || status === 'pending') {
          counts.gagal++;
        }
      });
      const parts = [];
      parts.push(`<span class="${counts.berhasil > 0 ? 'text-primary font-medium' : 'text-muted-foreground'}">${counts.berhasil} Berhasil</span>`);
      parts.push(`<span class="text-muted-foreground">${counts.proses} Diproses</span>`);
      parts.push(`<span class="${counts.gagal > 0 ? 'text-destructive' : 'text-muted-foreground'}">${counts.gagal} Gagal</span>`);
    ...
    getItemStatusCount(transaction, status) {
      ...
      return transaction.items.filter(item => {
        ...
        if (status === 'completed') {
          return ['berhasil', 'selesai', 'completed', 'success'].includes(itemStatus);
        } else if (status === 'processing') {
          return ['proses', 'processing'].includes(itemStatus);
        } else if (status === 'failed') {
          return ['gagal', 'failed', 'pending'].includes(itemStatus);
        }
        return false;
      }).length;
    },
    ...
    getItemStatusLabel(status) {
      ...
      if (['berhasil', 'selesai', 'completed', 'success'].includes(s)) return 'Sukses';
      if (['proses', 'processing'].includes(s)) return 'Diproses';
      if (['gagal', 'failed', 'pending'].includes(s)) return 'Gagal';
      return status;
    },

    getItemStatusBadgeClass(status) {
      const s = (status || '').toLowerCase();
      if (['berhasil', 'selesai', 'completed', 'success'].includes(s)) return 'badge-primary';
      if (['proses', 'processing'].includes(s)) return 'bg-yellow-500 text-white border-yellow-500';
      if (['gagal', 'failed', 'pending'].includes(s)) return 'badge-destructive';
